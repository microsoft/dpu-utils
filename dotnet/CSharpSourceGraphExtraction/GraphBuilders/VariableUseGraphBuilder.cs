...
namespace MSRC.DPU.CSharpSourceGraphExtraction.GraphBuilders
{
    ...
    internal class VariableUseGraphBuilder : CSharpSyntaxVisitor<(SyntaxToken? entryNode, IEnumerable<SyntaxToken> exitNodes)>
    {
        /// <summary>
        ...
        private VariableUseGraphBuilder(SourceGraph graph)
        {
            _graph = graph;
        }

        public static void AddVariableUseGraph(SourceGraph graph)
        {
            new VariableUseGraphBuilder(graph).Visit(graph.SemanticModel.SyntaxTree.GetRoot());
        }

        #region Internal helpers
        private bool AddEdges(SyntaxToken source, IEnumerable<SyntaxToken> targets)
        {
            _graph.VariableUseNodes.Add(source);
        ...
        private (SyntaxToken? entryNode, IEnumerable<SyntaxToken> exitNodes) ChainOptionalResults(IEnumerable<(SyntaxToken? entryNode, IEnumerable<SyntaxToken> exitNodes)> data)
        {
            ...
            foreach (var (entryNode, exitNodes) in data)
            {
                if (entryNode.HasValue)
                {
                    firstEntry = firstEntry ?? entryNode;
                    AddEdges(entryNode.Value, curExits);
                    curExits = exitNodes;
                }
            }
        ...
        {
            _graph.VariableUseNodes.Add(declaredId);
            _graph.VariableDeclarationNodes.Add(declaredId);
            _graph.VariableWriteNodes.Add(declaredId);
            if (initialValue != null)
            {
                var (initializerEntry, initializerExits) = Visit(initialValue);
                foreach (var usedToken in initialValue.DescendantTokens().Where(token => _graph.VariableUseNodes.Contains(token)))
                {
        ...
        private void HandleTupleAssignment(VariableDesignationSyntax lhsNode, SyntaxNode rhsNode, bool isDeclaration = false)
        {
            ...
            {
                var rhsUsedVars = rhsNode.DescendantTokens().Where(token => _graph.VariableUseNodes.Contains(token)).ToArray();
                foreach (var writtenToken in lhsNode.DescendantTokens().Where(token => _graph.VariableWriteNodes.Contains(token)))
                {
                    if (isDeclaration)
                    {
                        _graph.VariableDeclarationNodes.Add(writtenToken);
                    }
        ...
        public override (SyntaxToken? entryNode, IEnumerable<SyntaxToken> exitNodes) VisitPostfixUnaryExpression(PostfixUnaryExpressionSyntax node)
        {
            ...
            {
                var changedToken = RoslynUtils.GetAllVariableSymbolsInSyntaxTree(node, _graph.SemanticModel).FirstOrDefault();
                _graph.VariableWriteNodes.Add(changedToken);
            }
        ...
        {
            if (RoslynUtils.IsVariableLike(node, _graph.SemanticModel, out var _))
            {
                _graph.VariableUseNodes.Add(node.Identifier);
