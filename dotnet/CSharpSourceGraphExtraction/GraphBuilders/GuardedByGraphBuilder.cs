...
{
    internal class BlockGuardInformation
    {
        /// <summary>
        ...
        private ISet<string> GetUsedVariables(SyntaxNode node)
            => new HashSet<string>(node.DescendantTokens().Where(tok => _graph.VariableUseNodes.Contains(tok)).Select(tok => tok.Text));

        public void RecordValidatedGuard(SyntaxNode guardNode)
            => _validatedGuards.Add((guardNode, usedVariables: GetUsedVariables(guardNode)));
        ...
        public void RecordInvalidatedGuard(SyntaxNode guardNode)
            => _invalidatedGuards.Add((guardNode, usedVariables: GetUsedVariables(guardNode)));
    ...
    internal class GuardedByGraphBuilder : CSharpSyntaxWalker
    {
        private readonly SourceGraph _graph;
        ...
        {
            Visit(node.Condition);
            HandleConditionalBlock(new[] { node.WhenTrue }, validatedGuard: node.Condition);
            HandleConditionalBlock(new[] { node.WhenFalse }, invalidatedGuard: node.Condition);
        }
